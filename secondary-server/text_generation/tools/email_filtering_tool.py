...
from ..lib.load_data import batch_generator
...
def email_filtering_tool(
    ...
    results = []
    for temp_df in batch_generator():
        ...
        filtered_batch = temp_df.filter(mask)
    ...
    # --- Sorting ---
    final_df = pl.concat(results).sort(
        by=sort_by,
        descending=(sort_order.lower() == "desc")
    )

    # --- Preview results ---
    total_matches = final_df.height
    ...
    if limit is None:
        results_preview = final_df.select(preview_cols).to_dicts()
    else:
        results_preview = final_df.head(limit).select(preview_cols).to_dicts()
