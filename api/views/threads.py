...
class ThreadServiceView(APIView):
    def post(self, request):
        try:
            ...
            response = ThreadService.create_thread(data=validated_data)
    ...
    def put(self, request, thread_id: str):
        try:
            ...
            response = ThreadService.update_thread(
                thread_id=str(thread_id), data=validated_data
            )
    ...
    def get(self, request, thread_id: str):
        ...
        try:
            response: ThreadResponse = ThreadService.get_chat_thread_by_id(
            ...
            return Response(
                {"success": False, "Validation error": str(ve)},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except Exception as e:
            return Response(
...
class ListThreadServiceView(APIView):
    def get(self, request, user_id: str):
        ...
        try:
            ...
            thread_response: ThreadsResponse = ThreadService.list_chat_threads(
                user_id=str(user_id)
            )
            ...
            if thread_response.success:
                ...
                return Response(
                    {"success": True, "data": threads_data}, status=status.HTTP_200_OK
                )
            else:
                return Response(
