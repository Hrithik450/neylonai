...
class UserFormat(BaseModel):
    ...
    created_at: str
    assistant: Literal[
        "internal_assistant", "customer_service_assistant", "resume_assistant"
    ] = "internal_assistant"
...
class UserService:
    ...
    @staticmethod
    def deduct_tokens(
        deduction_tokens: int, user_id: str, resume_gen: bool = False
    ) -> UserResponse:
        try:
            ...
            user_response = UserFormat(
                ...
                name=str(name),
                email=str(email),
                emailverified=str(emailVerified),
                image=str(image),
                daily_limit=(
                    int(new_daily_limit) if not resume_gen else int(daily_limit or 0)
                ),
                resume_generation_limit=(
                    int(new_resume_limit)
                    if resume_gen
                    else int(resume_generation_limit or 0)
                ),
    ...
    def update_user(user_id: str, data: Dict[str, Any]) -> UserResponse:
        try:
            ...
            updated_user = UserFormat(
                id=str(row[0]),
                name=str(row[1]),
                email=str(row[2]),
                emailverified=row[3],
                image=str(row[4]),
                daily_limit=int(row[5]),
                resume_generation_limit=int(row[6]),
                created_at=str(row[7].isoformat()),
                assistant=str(row[8]),
                role=str(row[9]),
            )
