...
export function Navbar() {
  ...
  return (
    ...
    >
      <nav className="container mx-auto flex items-center max-lg:w-[max-content] bg-white/25 backdrop-blur-xs border border-gray-400/60 rounded-full p-2 xl:p-3">
        <div className="px-6 lg:max-w-[180px] xl:max-w-[240px] w-full flex items-center">
        ...
        <div className="flex flex-1 justify-center max-sm:hidden">
          {navLists.map((navItem) => (
            <div
              className="px-2 xl:px-3 text-black transition-all text-base md:text-base xl:text-lg cursor-pointer"
              key={navItem.id}
            >
              {navItem.label}
            </div>
        ...
        <div className="flex items-center gap-4 xl:gap-6 max-sm:justify-end max-sm:flex-1">
          ...
          <Button
            variant="default"
            className="rounded-full px-6 xl:px-10 py-6 text-lg cursor-pointer"
          >
            Sign up
          </Button>
        </div>
