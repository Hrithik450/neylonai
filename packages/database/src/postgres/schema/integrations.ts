 ...
 * - integration_type: catalog id (website, pdf, hubspot, …)
...
export const organizationIntegrations = pgTable(
  "organization_integrations",
  {
    ...
    integration_type: varchar("integration_type", { length: 64 }).notNull(),
  ...
  (t) => [
    uniqueIndex("organization_integrations_org_integration_type_uidx").on(
      t.organization_id,
      t.integration_type,
    ),
  ],
...
export const organizationIntegrationSecrets = pgTable(
  ...
  (t) => [
    ...
    index("organization_integration_secrets_organization_id_idx").on(
      t.organization_id,
    ),
  ],
);
