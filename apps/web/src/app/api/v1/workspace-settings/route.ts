...
export async function GET(req: NextRequest) {
  try {
    ...
    const settings = await getWorkspaceSettings(gate.org.organizationId);
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const body = (await req.json()) as WorkspaceSettingsPatch;
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        /** Shown once when rotateWebhookSecret was requested. */
        webhookSecretOnce: result.webhookSecretOnce ?? null,
      },
    });
  } catch (error) {
    return NextResponse.json(
